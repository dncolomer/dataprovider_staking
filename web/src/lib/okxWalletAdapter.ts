/**
 * OKX Wallet adapter — ported verbatim from the sister project
 * `unsys_staking/web/src/lib/okx-wallet-adapter.ts`, which is the
 * version that's proven to work in production.
 *
 * OKX ships no package in @solana/wallet-adapter-wallets. They support
 * the Solana Wallet Standard, so newer OKX versions are auto-detected
 * by WalletProvider — but older versions (and some OS/browser combos)
 * only expose `window.okxwallet.solana`. Registering this adapter
 * guarantees OKX always shows in the modal: "Installed" when detected,
 * with the install link otherwise.
 */
import {
